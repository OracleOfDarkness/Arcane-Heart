...
package net.mcreator.arcaneheart.item.inventory;
...
public class SpellbookInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	...
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == ArcaneHeartModItems.SPELLBOOK.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(0) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != ArcaneHeartModItems.SPELLBOOK.get();
			}
